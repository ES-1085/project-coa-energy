...
```
=======
I made a new dataset (new_data) where Turrets Annex is labeled Turrets:
...
```{r}
energy_use %>% 
 filter(Building == c("Turrets","Seafox","Turrets Annex", "Blair Tyson", "Davis Center" )) %>%
  select(Building , Gallons, `Delivery Date`) %>%
  arrange(`Delivery Date`) %>%
  ggplot() +
  geom_smooth(mapping = aes(x = `Delivery Date`, y = Gallons ))+
  facet_wrap(~Building)
```
```{r}
new_data %>%
  group_by(`Delivery Date`, Building) %>% 
  summarise(across(c(Building, `Delivery Date`), sum))
```

>>>>>>> a3b8c23f79c75aee6df01fe5c24f5fe31dca4988
